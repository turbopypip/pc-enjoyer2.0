...
const VideoCard = () => {
  return (
    <div>
      ...
      <p>
        В видеокартах, в отличии от процессоров, ядер тысячи. Их основная задача
        – параллельная работа над одной задачей – арифметическими вычислениями
        для отображения пикселей определённого цвета. В это же время ядра
        процессора работают отдельно над совершенно разными задачами.
      </p>
      <p>
      ...
      <p>
        Параметры трёхмерных объектов, наблюдаемые вами на мониторе изначально,
        просчитываются в векторном виде. Каждый такой объект состоит из
        треугольников, координаты которых в пространстве описываются их
        вершинами.
      </p>
      <p>
      ...
      <p>
        ...
        вершинами треугольников в пространстве накладывается текстура – цветная
        таблица пикселей определённого разрешения.
      </p>
      <p>
      ...
      <p>
        На сегодняшний день затенение трёхмерных объектов осуществляется за счёт
        карт – специальных текстур, содержащих необходимую информацию. Карта
        нормалей, например, хранит информацию о каждой точке поверхности
        шероховатого объекта.
      </p>
      <p>
        С её помощью описываются векторы проходящего света. Карта зеркальностей
        задаёт коэффициенты отражения различным частям текстуры. Карта
        шероховатостей придаёт объекту рельеф.
      </p>
      <p>
      ...
      <p>
        В конце всех вычислений, связанных с шейдингом, изображение отображается
        на экране.
      </p>
      ...
      <p>
        ...
        производительности или даже её приростом относительно того же разрешения.
      </p>
      <p>
      ...
      <p>
        ...
        настройках, от которых зависит потребляемая видеопамять. При игре с более высоким разрешением
          загруженность видеопамяти сильно зависит от игры.
        <br />
      </p>
      <p>
      ...
      <p>
        <img src={MoneyIcon} width="20" height="20" />
        Видеокарта при интенсивной нагрузке не только потребляет
        много энергии, но и серьезно нагревается. Перегрев может привести к
        снижению производительности, а в худшем случае к выходу графического
        адаптера из строя. Для избежания этого, производители используют системы
        охлаждения.
      </p>
      ...
      <p>
        ...
        материнскую плату не перекроет доступ к её компонентам. Видеокарта должна поместиться по длине и высоте в корпус вашего ПК. Параметр «Максимальная длина
        видеокарты» указывается в характеристиках практически всех
        компьютерных корпусов, а проблемы с высотой у вас возникнут только в
        случае сборки компактного ПК (с размером корпуса менее, чем ATX).
      </p>
      ...
      <p>
        HDMI и DisplayPort поддерживают все виды отображения, производительность,
        соответствующая уровню которых достижима современными мощностями
        компьютеров. VGA и DVI – их устаревшие аналоги, которые мы не советуем
