...
const Motherboard = () => {
  ...
  return (
    <div className="PageContainer">
      ...
      <p>
        <img src={MoneyIcon} width="20" height="20"/>Чипсет материнской платы обычно записывается производителями в цифробуквенном виде в названии продукта. Для покупателя основных отличия различных чипсетов три:
      </p>
      <p>
      ...
      <h3 className="contentParagraphTopics">Поддержка разгона процессора/оперативной памяти</h3>
      <p>
      ...
      <h3 className="contentParagraph" id="motherboardVRM">VRM материнской платы</h3>
      <p>
        <img src={MoneyIcon} width="20" height="20"/>При выборе материнской платы стоит учитывать характеристики питания VRM. Сами производители их почти нигде не указывают. Однако в сети существуют таблицы, в которых достаточно подробно указываются нужны для благополучного выбора платы.
      </p>
      <p>
      ...
      <h2 className="contentParagraph" id="motherboardPowerSystemCooling">Охлаждение системы питания</h2>
      <p>
        <img src={MoneyIcon} width="20" height="20"/>Важно помнить об охлаждении цепей VRM. Такое охлаждение представлено радиаторами в соответствующих областях – слева и/или сверху от сокета.
      </p>
      <p>
      ...
      <h2 className="contentParagraph" id="motherboardQVLCanalsTopology">QVL список, каналы и топология материнской платы</h2>
      <p>
        <img src={MoneyIcon} width="20" height="20"/>Наличие оперативной памяти в списке QVL, количество каналов и топология платы в разных степенях влияют на работоспособность оперативной памяти.
      </p>
      <p>
      ...
      <h2 className="contentParagraph" id="motherboardPowerSupply">Питание материнской платы</h2>
      <p>
        <img src={PhysicsIcon} width="20" height="20"/>Современные блоки питания в связи особенностями их наполнения сами по себе не способны отвечать электрическим требованиям процессора (низкое напряжение при высокой силе тока). В связи с этим на материнских платах устанавливаются VRM (модуль регуляции напряжения) преобразователи тока.
      </p>
      <p>
        Такие модули используют импульсные преобразователи напряжения. Смысл их работы заключается в том, чтобы распределить подачи тока высокого напряжения за единицу времени на части. Результат работы импульсного преобразователя – очень короткая подача тока высокого напряжения, которая приводит к тому, что за одну и ту же единицу времени подалось меньшее напряжение.
      </p>
      <p>
        Насколько коротка должна быть подача тока определяется имеющимся и требуемым напряжением. Например, чтобы подать за 1 секунду 3 вольта вместо 10, в течении 3/10 секунд преобразователь будет подавать ток.
      </p>
      <p>
        Однако только с помощью преобразователя процессор работать не может, ведь ток всегда будет прерывист и данные будут теряться. Для решения этой проблемы применяется LC-фильтр, состоящий из индуктивности и ёмкости.
      </p>
      <p>
        Индуктивность на материнской плате представлена дросселями, а ёмкость – конденсаторами. Выглядят они следующим образом:
      </p>
      <p>
        <img src={Photo8} className="motherboardPhoto8"/>
      </p>
      <p>
        Функция LC-фильтра – снижение или поднятия уровня напряжения в текущий момент времени в соответствии с желаемым.
      </p>
      <p>
        По включению компьютера логика контроллера (в остальное время питаемого батареей материнской платы) активируется и контроллер начинает работу. Он подаёт на связь драйвера и мосфета сигнал определённой частоты.
      </p>
      <p>
        Драйвер и мосфет в этом случае – ключевые элементы импульсного преобразователя, которые и выполняют функцию своеобразного таймера. Они принимают в себя ток от блока питания и сохраняют частоту, заданную контроллером, однако сила и мощность тока сигнала возрастает.
      </p>
      <p>
        После прохождения мосфета, сигнал встречает LC-фильтр, который сглаживает сигнал, а завершает путь заряда сам процессор.
      </p>
      <p>
        Цепочки из пар драйвер-мосфет и фильтров называют цепями питания. Если цепи имеют сдвиг по фазе относительно друг друга – импульс по ним проходит в разное время, то их называют фазами питания.
      </p>
      <p>
        Представить эту схему можно в следующем виде:
      </p>
      <p>
        <img src={Photo9} className="motherboardPhoto9"/>
      </p>
      <h2 className="contentParagraphTopics">3 Вида схем питания материнских плат</h2>
      <p>
        Если подавать ток с напряжением в 10 Вольт и 250 Ампер на лишь одну пару мосфет-драйвер, то он непременно не выдержит нагрузку и сгорит. Поэтому на материнских платах применяется в основном три вида организации питания процессора: прямая, даблерная, распараллеленная.
      </p>
      <p>
        Даблер – раздвоитель сигнала, уменьшающий частоту изначального сигнала контроллера вдвое у каждого из двух новых сигналов. Раздвоение сигнала позволяет подключить один канал контроллера к двум разным парам мосфет-драйвер и, следственно сдвигает по фазе так же, как если бы мосфет-драйвер был подключён напрямую или парно.
      </p>
      <p>
        Однако, уменьшенная вдвое частота вызывает негативный эффект – за одну и ту же единицу времени доступно меньше сдвигов по первоначальной фазе. Этот эффект компенсируется большей частотой сигналов контроллера.
      </p>

      <br/>

      <p>
        Даблер, как и сдвиги по фазе цепей питания, требуется для уменьшения отклонения, возникающего в результате работы LC-фильтра.
      </p>
      ...
      <p>
        При прямом виде подключения количество каналов контроллера равно количеству фаз и цепей питания, а даблеры отсутствуют.
      </p>
      ...
      <p>
        При распараллеленном виде подключения количество каналов контроллера равно количеству фаз, даблеры отсутствуют, а количество цепей вдвое больше фаз и каналов.
      </p>
      ...
      <p>
        При даблерном виде подключения количество каналов контроллера равно количеству даблеров, а количество цепей и фаз вдвое больше каналов и даблеров.
      </p>
      ...
      <p>
        Все три схемы рядом друг с другом:
      </p>
      ...
      <h2 className="contentParagraph" id="motherboardConnectors">Разъемы</h2>
      <h3 className="contentParagraphTopics">M.2 SSD</h3>

      <p>
        <img src={MoneyIcon} width="20" height="20"/>M.2 накопителей есть два главных отличия: по ключу подключения и по размеру.
      </p>

      <p>
        Накопители формата M.2 имеют различные ключи подключения к материнской плате. Соответственно, ключ накопителя должен соответствовать входу на материнской плате.
      </p>
      ...
      <p>
        Также вы должны учесть максимальную возможную длину подключаемых накопителей, информацию о которой вы можете найти в спецификации выбранной платы.
      </p>
      ...
      <h2 className="contentParagraph" id="motherboardExtraInformation">Дополнительные сведения</h2>
      <p>
        <img src={MoneyIcon} width="20" height="20"/>На некоторых материнских платах устанавливаются несколько микросхем BIOS, для того чтобы в экстренной ситуации вы смогли поменять их местами, и запустить ПК, не прибегая к помощи сервисных центров.
      </p>
      <p>
      ...
      <h2 className="contentParagraph" id="motherboardSelectionAlgorithm">Алгоритм выбора материнской платы</h2>
      <p>
        1. Определитесь с выбором процессора. Его сокет должен совпадать с сокетом материнской платы.
      </p>
      <h3 className="contentParagraphTopics">Если вы не имеете возможности в любом случае обновить BIOS платы:</h3>
      <p>
